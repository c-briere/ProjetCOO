...
public class TraitementReservation implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(o==this.fenetreGestionReservation.getBouttonNextChoixDate()){
			if(!this.fenetreGestionReservation.getDateArrive().getText().equals("") && !this.fenetreGestionReservation.getDateDepart().getText().equals("") && !this.fenetreGestionReservation.getNbPersonne().getText().equals("")){
				String jourDepart = this.connect.gestionDate.dateEnjours(this.fenetreGestionReservation.getDateDepart().getText());
				String jourArrivee=this.connect.gestionDate.dateEnjours(this.fenetreGestionReservation.getDateArrive().getText());
				...
				int cleLigneAller = this.connect.gestionLigne.cleLigne(this.fenetreGestionReservation.getVilleDepart().getNom(), this.fenetreGestionReservation.getChoixVille().getSelectedItem().toString());
				int cleLigneRetour =this.connect.gestionLigne.cleLigne(this.fenetreGestionReservation.getChoixVille().getSelectedItem().toString(),this.fenetreGestionReservation.getVilleDepart().getNom() );
				...
				int cleVilleArrive = this.connect.gestionVille.cleVille(this.fenetreGestionReservation.getChoixVille().getSelectedItem().toString());
		...
		if(o==this.fenetreGestionReservation.getBouttonNextChoixAller()){
			String heureAller = (String)this.fenetreGestionReservation.getTableau().getModel().getValueAt(this.fenetreGestionReservation.getTableau().getSelectedRow(),1);
			String classeAller;
			double prixAller;
			if(this.fenetreGestionReservation.getChoixClasse().getSelectedItem().toString().equals("1ere classe")){
				classeAller="1 ere classe";
				prixAller= (Double)this.fenetreGestionReservation.getTableau().getModel().getValueAt(this.fenetreGestionReservation.getTableau().getSelectedRow(),8);
			}
			else{
				classeAller="2 eme classe";
				prixAller= (Double)this.fenetreGestionReservation.getTableau().getModel().getValueAt(this.fenetreGestionReservation.getTableau().getSelectedRow(),9);
		...
		if(o==this.fenetreGestionReservation.getBouttonNextChoixRetour()){
			String heureRetour = (String)this.fenetreGestionReservation.getTableau().getModel().getValueAt(this.fenetreGestionReservation.getTableau().getSelectedRow(),1);
			String classeRetour;
			double prixRetour;
			if(this.fenetreGestionReservation.getChoixClasse().getSelectedItem().toString().equals("1ere classe")){
				classeRetour="1 ere classe";
				prixRetour= (Double)this.fenetreGestionReservation.getTableau().getModel().getValueAt(this.fenetreGestionReservation.getTableau().getSelectedRow(),8);
			}
			else{
				classeRetour="2 eme classe";
				prixRetour= (Double)this.fenetreGestionReservation.getTableau().getModel().getValueAt(this.fenetreGestionReservation.getTableau().getSelectedRow(),9);

			}
			conversationDate();
			chambreNonDispo=this.connect.gestionHotel.listChambreNonDipos(this.fenetreGestionReservation.getCleVilleArrive(), aller,retour);
			
			ArrayList<HotelRestant> hotel = this.connect.gestionHotel.listHotel(this.fenetreGestionReservation.getCleVilleArrive(), this.fenetreGestionReservation.getNbPersonneVoyage(), aller,retour,chambreNonDispo);
		...
		if(o==this.fenetreGestionReservation.getBouttonNextChoixHotel()){
			int idHotel;
			idHotel=this.connect.gestionHotel.cleHotel(this.fenetreGestionReservation.getHotel().get(this.fenetreGestionReservation.getChoixHotel().getSelectedIndex()).getNom(), this.fenetreGestionReservation.getCleVilleArrive());
			ArrayList<Categorie> categorie= new ArrayList<Categorie>();
			conversationDate();
			chambreNonDispo=this.connect.gestionHotel.listChambreNonDipos(this.fenetreGestionReservation.getCleVilleArrive(),aller,retour);

			categorie= this.connect.gestionCategorie.listCategorie(idHotel,chambreNonDispo,this.fenetreGestionReservation.getNbPersonneVoyage());
		...
		if(o==this.fenetreGestionReservation.getBouttonNextChoixCategorie()){
			int idCategorie;
			idCategorie=this.connect.gestionCategorie.cleCategorie(this.fenetreGestionReservation.getCategorie().get(this.fenetreGestionReservation.getChoixCategorie().getSelectedIndex()).getNom(), this.fenetreGestionReservation.getIdHotel());
			conversationDate();
		...
		if(o==this.fenetreGestionReservation.getBouttonNextChoixChambre()){
			...
			idChambre=this.connect.gestionChambre.cleChambre(this.fenetreGestionReservation.getChambre().get(this.fenetreGestionReservation.getChoixChambre().getSelectedIndex()).getDenomination(),this.fenetreGestionReservation.getIdCategorie());
			nbJour=this.connect.gestionDate.nbDeJour(this.fenetreGestionReservation.getDateAller(),this.fenetreGestionReservation.getDateRetour());
			Reservation reservation= this.connect.gestionReservation.createReservation(this.fenetreGestionReservation.getHeureAller(),this.fenetreGestionReservation.getHeureRetour(),this.fenetreGestionReservation.getVilleDepart().getNom(),this.fenetreGestionReservation.getDateAller(),this.fenetreGestionReservation.getVilleRetour(),this.fenetreGestionReservation.getDateRetour(),this.fenetreGestionReservation.getPrixAller(),this.fenetreGestionReservation.getPrixRetour(),this.fenetreGestionReservation.getNbPersonneVoyage(),this.fenetreGestionReservation.getIdHotel(),this.fenetreGestionReservation.getIdCategorie(),this.fenetreGestionReservation.getCleClient(),idChambre,nbJour);
		...
		if(o==this.fenetreGestionReservation.getBouttonValider()){
			boolean verif =this.connect.gestionReservation.ajoutResa(this.fenetreGestionReservation.getCleClient(),this.fenetreGestionReservation.getCleVilleDepart(),this.fenetreGestionReservation.getCleVilleArrive(),this.fenetreGestionReservation.getCleLigneAller(),this.fenetreGestionReservation.getCleLigneRetour(),this.fenetreGestionReservation.getClasseAller(),this.fenetreGestionReservation.getClasseRetour(),this.fenetreGestionReservation.getIdHotel(),this.fenetreGestionReservation.getIdCategorie(),this.fenetreGestionReservation.getCleChambre(),this.fenetreGestionReservation.getDateAller(),this.fenetreGestionReservation.getDateRetour(),this.fenetreGestionReservation.getReservation().getPrixTotal(),this.fenetreGestionReservation.getReservation().getNbPersonne());
			if(verif){
